...
public final class MarkdownMessageConverter extends MessageConverter {
    ...
    public MarkdownMessageConverter() {
        ...
        this.renderPolicy = new RenderPolicy(new FlexmarkAnsiRenderer(), false);
    }
    ...
    public MarkdownMessageConverter(MarkdownRenderer renderer, boolean markedOnly) {
        this.configured = new Configuration(renderer, markedOnly);
        this.renderPolicy = new RenderPolicy(renderer, markedOnly);
    }
    ...
    public void start() {
        ...
        boolean markedOnly = (configured != null && configured.markedOnly())
                || ConverterOptions.hasFlag(options, MARKED_OPTION);
        MarkdownRenderer renderer = configured == null
                ? defaultRenderer(options)
                : configured.renderer();
        renderPolicy = new RenderPolicy(renderer, markedOnly);
        super.start();
    }
    ...
    public String convert(ILoggingEvent event) {
        ...
        if (policy.markedOnly() && !MarkdownMarkers.contains(event.getMarkerList())) {
            return rawMessage;
        }
        try {
            return preserveOuterLineBreaks(rawMessage, pad(policy.renderer().render(rawMessage)));
        }
    ...
    private MarkdownRenderer defaultRenderer(List<String> options) {
        ...
        AnsiPolicy ansiPolicy = plain
                ? AnsiPolicy.NEVER
                : ansi ? AnsiPolicy.ALWAYS : AnsiPolicy.DETECT;
        int tableWidth = ConverterOptions.intValue(
                options,
                TABLE_WIDTH_OPTION,
                "%mdMsg",
                MarkdownRendererOptions.DEFAULT_TABLE_WIDTH,
                MarkdownRendererOptions::isValidTableWidth,
                this::addWarn);
        return new FlexmarkAnsiRenderer(MarkdownRendererOptions.defaults()
                .withAnsiPolicy(ansiPolicy)
                .withTableWidth(tableWidth)
                .withTableIndent(LEFT_PADDING.length())
                .withPalette(ConverterOptions.palette(
                        options, "%mdMsg", this::addWarn, this::addWarn)));
    }
    ...
    private record Configuration(MarkdownRenderer renderer, boolean markedOnly) {
    }

    private record RenderPolicy(MarkdownRenderer renderer, boolean markedOnly) {
    }
}
