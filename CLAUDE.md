...
| `inquisitor-harness-evaluation-report` | The evaluation report: a `LauncherSessionListener` writes it — bucket-grouped, expectation-aware — when `inquisitor.report.dir` is set (the plugin's `evaluate` task sets it). Default format is multi-page HTML (`evaluation.html` → `buckets/*.html` → `scenarios/*.html`, Gradle-report-style navigation); `markdown` and `json` selectable via the task's `--report` option. Renderers are pluggable via `ServiceLoader` (`EvaluationReportRenderer`: `name()` + `render(report, dir)`). Shipped by the evaluation starter by default; excludable (`@ConditionalOnClass`-guarded registration) for score-only evaluation |
